...
def get_the_data():
    ...
    for x in league_info_list:
        ...
        league_data[x[0]].html_standings = mocks.standings_mock[x[0]]
        league_data[x[0]].html_prev_week = mocks.prev_week_mock[x[0]]
        league_data[x[0]].html_cur_week = mocks.cur_week_mock[x[0]]

    for x in league_data:
        #print leagues[x].html_standings
        #print leagues[x].html_prev_week
        #print leagues[x].html_cur_week
        league_data[x].teams = parsers.parse_standings(league_data[x].html_standings)
        #parsers.parse_prev_week(leagues[x].html_prev_week)
        #parsers.parse_cur_week(leagues[x].html_cur_week)

    return league_data


leagues = get_the_data()
sorted_leagues = []
